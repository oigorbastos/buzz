...
impl InnerWebView {
  ...
  fn init_webview(
    ...
    // even when `ipc_handler` is `None`, which is an unnecessary attack surface
    // for remote-only child webviews.
    if attributes.ipc_handler.is_some() {
    ...
    if let Some(handler) = pl_attrs.resource_request_handler {
      unsafe {
        Self::attach_resource_request_handler(&webview, env, handler, &mut token)?;
      }
    }
  ...
  #[inline]
  unsafe fn attach_resource_request_handler(
    webview: &ICoreWebView2,
    env: &ICoreWebView2Environment,
    handler: std::sync::Arc<dyn Fn(String) -> bool + Send + Sync>,
    token: &mut EventRegistrationToken,
  ) -> Result<()> {
    let filter = HSTRING::from("*");
    let webview_22 = webview.cast::<ICoreWebView2_22>()?;
  ...
  #[inline]
  fn disable_file_chooser(webview: &ICoreWebView2) -> Result<()> {
        ...
        Box::new(move |handler| {
          unsafe {
            webview.CallDevToolsProtocolMethod(
              w!("Page.setInterceptFileChooserDialog"),
              w!(r#"{"enabled":true,"cancel":true}"#),
              &handler,
            )?;
          }
          Ok(())
        ...
        Box::new(|result, response| {
          ...
          if response.contains("\"error\"") {
            return Err(windows::core::Error::new(
              E_FAIL,
              "WebView2 refused file chooser interception",
            ));
          }
          Ok(())
        }),
  ...
  #[inline]
  unsafe fn disable_external_uri_schemes(
    webview: &ICoreWebView2,
    token: &mut EventRegistrationToken,
  ) -> Result<()> {
    let webview_18 = webview.cast::<ICoreWebView2_18>()?;
    webview_18.add_LaunchingExternalUriScheme(
