...
pub trait WebViewBuilderExtWindows {
  ...
  /// Set an allow/deny handler for every WebView2 resource request.
  ///
  /// Returning `false` replaces the request with a local 403 response. This is
  /// useful for remote-only children that must not probe app-local services.
  fn with_resource_request_handler(
    self,
    handler: impl Fn(String) -> bool + Send + Sync + 'static,
  ) -> Self;

  /// Prevent HTML file controls from opening a native chooser.
  ///
  /// Creation fails closed if the installed WebView2 runtime cannot install
  /// the DevTools protocol interception before the first navigation.
  fn with_file_chooser_disabled(self, disabled: bool) -> Self;

  /// Prevent the page from launching non-HTTP(S) URI schemes through Windows.
  ///
...
impl WebViewBuilderExtWindows for WebViewBuilder<'_> {
  ...
  fn with_resource_request_handler(
    ...
    self.platform_specific.resource_request_handler = Some(Arc::new(handler));
    self
  }

  fn with_file_chooser_disabled(mut self, disabled: bool) -> Self {
    self.platform_specific.file_chooser_disabled = disabled;
    self
  }
