...
mod postgres_tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        // Versions 1–34 include migrations already applied in this fork's
        // production database (the fork's own 0029–0031 lab-board migrations
        // shift everything upstream added afterwards by three). Colliding
        // upstream migrations are appended at the tail instead of renumbering
        // history: upstream 0032–0034 (roster fence, private managed-agent
        // FTS, replica heartbeat vacuum) live here as 0043–0045, and upstream
        // 0043–0045 (push dogfood profile, NIP-FI ledger drop, push revocation
        // tombstones) live here as 0046–0048. 0035–0042 keep their upstream
        // versions.
        ...
        // NIP-PMA kind:30179 FTS exclusion (0044): same wrap-the-existing-
        ...
        // GIN index — see the 0044 header for the operational cost.)
        assert_eq!(migrations[43].version, 44);
        assert!(migrations[43].sql.as_str().contains("kind = 30179"));
        ...
        // These three Lab migrations are already applied in production and
        // therefore occupy their original 0029–0031 slots permanently.
        assert_eq!(migrations[28].version, 29);
        let lab_boards = migrations[28].sql.as_str();
        assert!(lab_boards.contains("CREATE TABLE lab_board_heads"));
        assert!(lab_boards.contains("CREATE TABLE lab_board_revisions"));
        assert_eq!(migrations[29].version, 30);
        let lab_board_d_tag_backfill = migrations[29].sql.as_str();
        assert!(lab_board_d_tag_backfill.contains("WHERE kind = 40101"));
        ...
        // fence. It was upstream 0029 but is 0032 here because 0029–0031
        // are pinned, already-applied Lab migrations.
        ...
        // Recovery migration 0033 alters populated tables and must preserve
        ...
        let roster_fence = migrations[42].sql.as_str();
        ...
        // fence as migration 0043. CI and isolated relay startup use schema.sql
        ...
        let heartbeat_vacuum = migrations[44].sql.as_str();
        ...
        // Upstream 0043; renumbered to 0046 in this fork (see
        // embedded_migrator_contains_consolidated_initial_schema).
        assert_eq!(migrations[45].version, 46);
        let dogfood_profile = migrations[45].sql.as_str();
        ...
        // removed relations.
        // Upstream 0044; renumbered to 0047 in this fork.
        assert_eq!(migrations[46].version, 47);
        let ledger_removal = migrations[46].sql.as_str();
    ...
    fn deletion_surface_parity_between_migration_0032_and_schema_sql() {
        ...
        // Upstream's community-deletion migration is 0029, but this fork's
        // already-applied Lab migrations occupy 0029–0031.
    ...
    async fn populated_upgrade_preserves_search_policy_except_for_private_kinds() {
        ...
        // tokenized kind:30179 ciphertext — the gap 0044 closes.
        MIGRATOR
            .run_to(43, &pool)
            .await
            .expect("apply migrations through 43");
        let pre_0044: Vec<(i32, Option<bool>)> = sqlx::query_as(
            "SELECT kind, search_tsv @@ plainto_tsquery('simple', 'needle') \
             FROM events ORDER BY kind",
        )
        .fetch_all(&pool)
        .await
        .expect("read pre-0044 search behavior");
        assert_eq!(
            pre_0044,
    ...
    /// Verify migration 0047 (upstream 0044) applies cleanly against a DB
    /// that has rows in the NIP-FI 0041+0042 tables. The immutability guards (no_delete,
    /// no_truncate) are enforced via triggers; DROP TABLE bypasses them and
    /// must succeed even when rows are present.
    #[tokio::test]
    #[ignore = "requires Postgres"]
    async fn migration_0047_drops_populated_nip_fi_ledger_cleanly() {
        let pool = connect_test_pool().await;
        reset_public_schema(&pool).await;
        MIGRATOR
            .run_to(42, &pool)
        ...
        // Apply the fork's 0043-0045, then 0046 (dogfood profile) and
        // 0047 (ledger removal).
        MIGRATOR
            .run_to(47, &pool)
            .await
            .expect("migration 0047 must apply cleanly against a populated NIP-FI DB");
        ...
        assert!(
            present.is_empty(),
            "all NIP-FI tables must be absent after migration 0047: {present:?}"
        );
