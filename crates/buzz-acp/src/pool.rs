...
mod tests {
    ...
    // Fork-only: the typed Claude Code policy (`claudeCode.options`) that
    // admits the reply command and the narrow `buzz lab` family. Kept inline
    // so `pool/system_prompt_tests.rs` stays byte-identical to upstream.
    #[test]
