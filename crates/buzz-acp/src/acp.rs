...
mod tests {
    ...
    // Fork-only: `session_new_full_with_claude_options` must merge the typed
    // Claude Code allowlist (`_meta.claudeCode.options.allowedTools`) with the
    // ClaudeMeta system prompt and the session title in one `_meta` object.
    // Kept inline so `acp/system_prompt_tests.rs` stays byte-identical to
    // upstream; the shared helpers live there.
    #[tokio::test]
